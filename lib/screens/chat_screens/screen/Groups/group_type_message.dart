...
import 'package:vibetag/screens/chat_screens/screen/Groups/view_group.dart';
...
class _GroupTypeMessageState extends State<GroupTypeMessage> {
  ...
  sendGroupMessage() async {
    var headers = {
      'Cookie':
          'PHPSESSID=d42e0993178c0413072e5c5757e7f5da; _us=1675947886; access=1; ad-con=%7B%26quot%3Bdate%26quot%3B%3A%26quot%3B2023-02-08%26quot%3B%2C%26quot%3Bads%26quot%3B%3A%5B%5D%7D; mode=day; post_privacy=0; src=1'
    };
  ...
  getGroupMessage() async {
    var headers = {
      'Cookie':
          'PHPSESSID=d42e0993178c0413072e5c5757e7f5da; _us=1675940843; access=1; ad-con=%7B%26quot%3Bdate%26quot%3B%3A%26quot%3B2023-02-08%26quot%3B%2C%26quot%3Bads%26quot%3B%3A%5B%5D%7D; mode=day; post_privacy=0; src=1'
    };
    var request = http.MultipartRequest(
        'POST', Uri.parse('https://vibetag.com/app_api.php'));
    request.fields.addAll({
    ...
    if (response.statusCode == 200) {
      ...
      final todo = tempList
              ?.map((dynamic item) => GroupModelList.fromJson(item))
              .toList() ??
          [];
      todo;
      group = todo;
  ...
  Future<void> get_all_membersIds() async {
    channel = getRandString(12);

    final result = await API().postData({
      'type': 'calling',
      'action': 'get_group_members_ids',
      'user_id': loginUserId.toString(),
      'group_id': group_id.toString(),
    });
    user_ids = jsonDecode(result.body)['user_ids'];
    for (var id in user_ids) {
      if (id.toString() != loginUserId.toString()) {
        await API().createAlertPush(
          body:
              'New Video Call From ${widget.list[widget.currentIndex].group_name}',
          title: 'New Video Call',
          fcm: '${id}',
          mapData: {
            'name': loginUser['name'],
            'avatar':
                getFullLink(widget.list[widget.currentIndex].avatar.toString()),
            'username': widget.list[widget.currentIndex].group_name.toString(),
            'user_id': loginUserId.toString(),
            'rec_id': group_id,
            'call_type': '1',
            'channel': channel,
    ...
    remote_user_data = {
      'name': loginUser['name'],
      'avatar': getFullLink(widget.list[widget.currentIndex].avatar.toString()),
      'username': widget.list[widget.currentIndex].group_name.toString(),
      'user_id': loginUserId.toString(),
      'rec_id': group_id,
      'call_type': '1',
    ...
    return SafeArea(
      child: Scaffold(
          appBar: PreferredSize(
            preferredSize:
                Size(double.maxFinite, screenHeightSize(70, context)),
            child: AppBar(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                widget.list[widget.currentIndex].group_name
                                    .toString(),
                                style: TextStyle(
